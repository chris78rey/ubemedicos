Este `web_frontend/pages/register.vue` queda alineado con el `login.vue` existente del proyecto y con los endpoints reales `/auth/register/patient` y `/auth/register/professional` del backend.  
...
Hay un detalle importante: el backend real acepta esos dos endpoints y valida `email`, `password`, `first_name`, `last_name` y `phone`; además, para profesional crea un perfil inicial en estado `DRAFT`. 

El prompt del usuario pide exactamente ese payload mínimo, y ese archivo ya queda listo para copiar y pegar.
