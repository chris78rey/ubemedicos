...
* `web_frontend/pages/professional/index.vue`
...
* El cambio depende de que ya existan y respondan estas rutas:
  ...
  * `PATCH /api/v1/professional/documents/<id>`
...
* La cookie del token se llama **`access_token`**.
* `apiBaseUrl` apunta a `/api/v1`.
* El hueco principal del snapshot actual está en que **ya existe backend del profesional para perfil y validación, pero todavía no existe la pantalla web que lo consuma**.
...
**Ruta base esperada:**
`G:\codex_projects\ubemedicos\web_frontend`

**Backup conservador:**

```bash
copy web_frontend\pages\professional\index.vue web_frontend\pages\professional\index.vue.bak
```
...
### Fase 1: Crear panel profesional de perfil y validación

**Objetivo:**
Cerrar el tramo faltante del profesional para que pueda:

* editar su perfil,
* subir o reemplazar documentos,
* ver bloqueos,
* consultar el estado de validación,
* reenviar el expediente,
* revisar la trazabilidad del proceso.

**Ruta del Archivo:**
`web_frontend/pages/professional/index.vue`

**Acción:**
**Crear** o **Reemplazar completo**
...
1. Ingresar con usuario profesional.
2. Abrir `/professional`.
3. Verificar que cargan perfil, especialidades, documentos y eventos.
4. Cambiar ciudad o licencia y guardar.
5. Subir documento `id`, `title` y `license` o `senescyt`.
6. Confirmar que desaparecen bloqueos cuando corresponde.
7. Presionar **Enviar expediente a validación**.
8. Verificar que el estado pase a `submitted`.
9. Abrir un documento desde la tabla y confirmar que el PDF se visualiza.
...
```bash
del web_frontend\pages\professional\index.vue
```

o restaurar backup:

```bash
copy /Y web_frontend\pages\professional\index.vue.bak web_frontend\pages\professional\index.vue
```
...
Se debe comprobar que nada de esto se rompa:

* `/professional/availability`
* `/professional/appointments`
* `/admin/professional-verifications`
* `/admin/index`
* flujo de login y lectura de cookie `access_token`

---

## 📉 Plan de Reversión Total

1. Restaurar `web_frontend/pages/professional/index.vue` desde `.bak`.
2. Levantar frontend nuevamente.
3. Verificar que las rutas previas siguen funcionando.
4. Como no hubo cambios en base de datos ni backend, no se requiere rollback SQL.

El bloque faltante más claro del snapshot quedó cubierto con esta pantalla. Si el menú lateral todavía no apunta a `/professional`, quedará pendiente solo el enlace de navegación.
